            ...
            with lock:
                ...
                if db_file.exists():
                    ...
                    if smiles in existing_smiles:
                        logger.warning(f"SMILES '{smiles}' already exists in database, skipping")
                        return False
                    ...
                    logger.info(f"Appending new entry with SMILES '{smiles}' to database")
                    new_row_df.to_csv(
                        db_path, 
                        mode='a',           # append mode
...
def validate_database_schema(db_path: str, expected_schema: List[str]) -> bool:
    ...
    try:
        db_file = Path(db_path)
        if not db_file.exists():
            logger.debug(f"Database file does not exist, schema validation passed: {db_path}")
            return True
