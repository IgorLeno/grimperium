...
class TestInteractiveBatchWorkflow:
    ...
    def test_convert_molecules_to_smiles_success(self, workflow):
        ...
        with patch(
            "grimperium.ui.interactive_batch.get_molecule_smiles"
        ) as mock_smiles:
            mock_smiles.side_effect = lambda mol, config: {
                "ethanol": "CCO",
                "methanol": "CO",
            }[mol]
    ...
    def test_validate_database_duplicates_no_duplicates(self, workflow):
        ...
        with patch(
            "grimperium.services.database_service.get_existing_smiles"
        ) as mock_db:
    ...
    def test_validation_steps_complete_success(self, workflow):
        ...
        with patch.object(workflow, "_convert_molecules_to_smiles", return_value=True):
            with patch.object(
                workflow, "_validate_database_duplicates", return_value=True
            ):
                with patch.object(
                    workflow, "_validate_pubchem_availability", return_value=True
                ):
    ...
    def test_validation_steps_database_validation_failure(self, workflow):
        ...
        with patch.object(workflow, "_convert_molecules_to_smiles", return_value=True):
            with patch.object(
                workflow, "_validate_database_duplicates", return_value=False
            ):
    ...
    def test_validation_steps_pubchem_validation_failure(self, workflow):
        ...
        with patch.object(workflow, "_convert_molecules_to_smiles", return_value=True):
            with patch.object(
                ...
                with patch.object(
                    workflow, "_validate_pubchem_availability", return_value=False
                ):
